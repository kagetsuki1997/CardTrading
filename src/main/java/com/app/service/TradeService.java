...
public class TradeService {
    ...
    public void makeAllDeal() {
        ...
        for (int i = 0; i < tradeList.size(); i++) {
            ...
            if (optTrade.isPresent()) {
                ...
                // Log.info("Deal with "+trade.getId()+" "+trade.getAction()+"
                // "+trade.getTradeCard().getName()+trade.getPrice());
    ...
    public Trade findABuyTrade(Trade trade) {
        Trade targetTrade = null;
        List<Trade> sellTradeList = tradeRepository.findSellTrade(trade.getTrader().getId(),
                trade.getTradeCard().getId(), trade.getPrice());
